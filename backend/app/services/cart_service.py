    ...
    def get_cart_details(self, cart_data: Dict[int, int]) -> CartResponse:
        ...
        for product_id, quantity in cart_data.items():
            if product_id in products_dict:
                ...
                cart_item = CartItem(product_id=product_id, quantity=quantity, name=product.name, price=product.price, subtotal=subtotal, image_url=product.image_url)
                cart_items.append(cart_item)
                total_price += subtotal
                total_items += quantity

        return CartResponse(items=cart_items, total=round(total_price), items_count=cart_items)
